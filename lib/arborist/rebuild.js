...
module.exports = cls => class Builder extends cls {
  ...
  async [_addToBuildSet] (node, set) {
    ...
    const needsGypBuild = !install && !preinstall && await isNodeGypPackage(node.path)
    if (bin || preinstall || install || postinstall || needsGypBuild) {
      if (bin)
        await this[_checkBins](node)
      if (needsGypBuild) {
        node.package.scripts = node.package.scripts || {}
        node.package.scripts.install = defaultGypInstallScript
      }
      set.add(node)
    }
  }
