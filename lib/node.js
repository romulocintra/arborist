...
const _changePath = Symbol('_changePath')
...
class Node {
  ...
  set root (root) {
    ...
    if (root !== this) {
      ...
      const loc = relpath(root.path, this.path)
      ...
      for (const p of walkUp(dirname(this.path))) {
        const ploc = relpath(root.path, p)
        const parent = root.inventory.get(ploc)
        if (parent) {
          const childLoc = `${p}${p ? '/' : ''}node_modules/${this.name}`
      ...
      const isChild = n => n.path === resolve(nm, n.name)
      // check dirname so that /foo isn't treated as the fsparent of /foo-bar
      const isFsChild = n => dirname(n.path).startsWith(this.path) && !isChild(n)
      const isKid = n => isChild(n) || isFsChild(n)
      for (const child of root.inventory.filter(isKid)) {
        // fsChildren just get removed, but children get a new parent
        if (!this.isLink && isChild(child)) {
  ...
  set fsParent (fsParent) {
    ...
    // nothing to do
    if (oldFsParent === fsParent)
      return

    const pathChange = newPath !== this.path
    const rootChange = fsParent.root !== this.root
  ...
  replace (node) {
    ...
    if (parent)
      parent.children.set(this.name, this)
    const fsParent = node[_fsParent]
    this[_fsParent] = fsParent
    if (fsParent) {
      fsParent.fsChildren.delete(node)
  ...
  set parent (parent) {
    ...
    const pathChange = newPath !== this.path
    const rootChange = parent.root !== this.root
  ...
  [_changePath] (newPath) {
    ...
    for (const child of this.fsChildren) {
      const relPath = relative(oldPath, child.path)
      child[_changePath](resolve(newPath, relPath))
    }
